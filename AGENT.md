...
이 문서는 최종 예측 모델을 `capacity_predictor_tool`로 연결해 자연어 분석 요청을 해석하고, cycle-level capacity/SOH 예측, 유사 degradation pattern 검색, 신뢰도 경고, 리포트 생성을 수행하는 후속 Phase 6 시스템을 설명한다.
...
- Active dataset is now Kaggle `patrickfleith/nasa-battery-dataset`.
- Raw data path: `data/nasa_battery_raw/cleaned_dataset`.
- Processed training table: `data/processed/nasa_cycle_level.csv`.
- Active target: `capacity`.
- Active default feature set: `discharge_summary`.
- Active split policy: `battery_id` group holdout. Never use random row split for cycle rows.
- The previous dataset track has been retired from active experiments.
